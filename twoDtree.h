...
private:
   ...
   */
   ...
   //added functions
   void split(stats & s, pair<int,int> ul, pair<int,int> lr,
   pair<int,int> & nUl, pair<int,int> & nLr);
   void renderHelper(PNG & canvas, Node * r);
   Node * copyHelper(Node * oRoot);
  void clearHelper(Node * & cRoot);
  void pruneHelper(Node * cNode, double pct, int tol);
  long distanceFromMean(Node * pRoot, Node * other);
  int numLeaves(Node * pRoot, Node * cRoot, int tol, int&);
  bool shouldPrune(Node * cNode, int tol, double pct);
